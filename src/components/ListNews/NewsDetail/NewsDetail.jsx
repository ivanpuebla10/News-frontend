...
const NewsDetail = () => {
  ...
  const image = news?.images?.map((image, i) => {
    if (fetched) {
      return (
        <Carousel.Item key={i}>
          <Card.Img src={image} className="carousel-image" />
        </Carousel.Item>
      );
    } else {
      return (
  ...
  return (
    <Card className="container">
      <Card.Body>
        ...
        <Carousel>{image ? image : null}</Carousel>
        <Card.Text className="content-container">{news?.content}</Card.Text>
      </Card.Body>
      <Card.Footer className="footer">
        <span>Author: {news?.author}</span>
        <span>{newDate ? newDate[0] : null}</span>
      </Card.Footer>
    </Card>
  );
};
